...
def upload_dataset(db: Session, file: UploadFile, name: str, description: str = "", owner: User | None = None) -> Dataset:
    ...
    流程：读取 -> 创建 Dataset 记录 -> 建表入库 -> 更新统计信息。
    """
...
def list_datasets(db: Session, user: User | None = None) -> list[Dataset]:
    """列出当前用户可见的数据集（按 owner 过滤，管理员可见全部）。"""
...
def delete_dataset(db: Session, dataset_id: int, user: User | None = None) -> bool:
    ...
    if not dataset:
        return False
    try:
        drop_table_if_exists(dataset.table_name)
    except Exception:  # noqa: BLE001
        logger.warning(
            "dataset.drop_table_failed",
            extra={"dataset_id": dataset_id, "table_name": dataset.table_name},
            exc_info=True,
        )
    db.delete(dataset)
    db.commit()
